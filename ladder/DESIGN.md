...
| Updates queue | `<ladder-updates>`, `.updates-queue`, `.updates-row` | **The single application-progress surface** (v2.29 — replaces live banners + the Needs-Attention widget; v2.32 — moves to the top of the **Inbox**, making Inbox the "Today" surface: what happened / what needs you above, new roles below). One elevated card, hairline-divided rows. Role closures are **server-backed events** (v2.34): check-liveness archives the role with `exit_reason='role_closed'` and inserts a `role_closed` application event — the queue row shows for 7 days until dismissed, and the dismissal persists server-side across devices (never hidden implicitly by page visits). Easy-apply digest lives here as well. **Auto-created roles (v2.38):** an application-receipt email that matches no pipeline role (user applied outside Ladder, straight on an ATS) auto-creates the role at stage `applied` (confidence ≥0.8, block-list respected) and surfaces as a `role_created` row ("Added {company} — application detected in Gmail", Open role action). **Type batching (v2.33):** same-kind rows collapse into one group row — count badge on the icon, "Show all N" expander opening an inset drawer of the individual rows (each keeps its own action + ×), group × dismisses the whole batch. A group of one renders as a plain row. The system acts proactively at scan time (offer ≥0.85 → stage Offer; rejection ≥0.75 → Archive with a stage-mapped exit_reason; 30d applied-quiet → Archive as no-response) and each row is a record of what it did with exactly **one action** — usually **Undo** (works any time from the feed via the event's stored `prev_state`, not a toast window; undoing an offer/rejection action re-opens it as a prompt so the decision stays on the table — × it away if unwanted). Below-floor classifications become one-click prompts ("Archive role" / "Move to offer"); the "low confidence" caption appears only when the event is genuinely below the auto floor; reply-pending rows open the Gmail thread; stale (14–30d) rows offer Follow up. × = acknowledge, persisted server-side (`dismissed_at`; localStorage only for synthetic rows). Auto-action records age out of the feed after 7 days. Icons are inline SVG line icons in 32px tinted circles — never emoji. |
| Signal chips | `.signal-chip`, `.signal-chip__icon` | One taxonomy everywhere: the table Signal column chip mirrors the role's Updates row (same icon + tint + short label, e.g. "Offer — confirm", "Reply pending", "Quiet"). Chips are buttons — clicking scrolls to and highlights the queue row (falls back to the role page). Colors: green offer · red rejection/archive · amber reply · blue calendar/advance · gray quiet. |
| Review & submit | `.ar`, `.ar-overlay`, `ladder-apply-review` | Easy Apply review-then-submit takeover (16.2c). Itemizes every field + answer the application will send, missing answers fill inline, and each legal/consent box is ticked by hand — nothing auto-checks. Submit is one explicit tap with no undo. Routes through the server transport when `SUBMIT_ENABLED` is on (Greenhouse v1); otherwise, and whenever a CAPTCHA is enforced or the POST is rejected, it shows the assisted panel: deep link + every prepared answer to copy. |
| Apply-ease chip | `.ease-chip`, `.ease-filter-pill`, `.role-header__apply-req` | Easy Apply badge (Phase 16.1): tier chip after the role title on Saved/Drafting rows — green ⚡ `easy` (no written questions) · amber `short_answer` · gray `essay` · dashed `special`/`portal`. Tooltip carries the form breakdown (ATS, question count, cover letter). No chip = unclassified. Meta-bar pill filters Saved/Drafting to easy-tier only (`?ease=easy`); the detail header shows the chip + a one-line "Application asks for:" breakdown. Icons are inline SVG line icons, same set style as `.signal-chip`. |
| Added column + filter | `.col-added`, `.added-filter` | Saved bucket only (v2.43): "Added" column shows `first_seen` as a compact muted date ("Jul 9", year suffix when not current), sortable like any column (default desc), full date in the cell tooltip. Meta-bar select filters by added-date range (`?added=1d\|7d\|30d\|30d+`), accent-tinted when active; empty result offers a one-tap Clear filter. Hidden on phones — full date lives on the role detail page. |
| Archive exit reason | `.archive-modal`, `.archive-modal__reasons` | The "why are you archiving?" modal appears only when archiving an **in-progress** (Active-stage) role — the outcome tunes future recommendations. Archiving straight from **Saved** skips the modal and archives immediately (never applied, no outcome to record); the server enforces the same rule (v2.44 / jobs-pipe 0.19). Auto-archives (role closed, no response, rejection) always carry a machine-set `exit_reason`. |
| Banners & toasts | `.toast-host`, `.recs-health-banner` | Every transient surface needs a defined lifecycle — at least two of: auto-timeout, explicit dismiss, state-driven refresh. Toasts (`job:toast`) render only through the global host in app.js (4.5s timeout, click-dismiss) and support an optional inline action (`{ action, onAction, duration }` — used for Undo confirmations, 8s). Confirmation banners auto-expire (added 12s, liveness 20s). Persistent alerts (source health) get ✕ = 6h signature-keyed snooze + 5-min background revalidation, and clear when the server state clears. Dismissals that must survive reloads persist to localStorage (`job:jobs:dismissedUpdates` for synthetic queue rows). |
...
- `.recs-health-banner` (error-colored) — a source is **blind**: dead Gmail token (`needsReauth` → "Reconnect Gmail" button) or a `lastError`. Distinct from "no new recs". Credit-outage variant (v2.38): a `lastError` matching the Anthropic "credit balance is too low" / "AI extraction temporarily unavailable" signature renders its own message ("AI extraction is paused") with an **Add credits** link to the Anthropic console billing page — every AI stage degrades together on this failure, so it outranks the generic source-issue copy.
- `.recs-draining` (accent-colored, with `.recs-draining__spinner`) — the backend is **actively scanning Gmail**. Determinate variant (v2.45): when the gmail health row carries catch-up counters (`backlogTotal`/`backlogLeft`, stamped per committed run by gmail-jobs into `job.gmail_scan_state`), the strip shows "Catching up on your email — X of Y" with a thin `.recs-draining__bar`; without counters it stays the indeterminate spinner ("Checking your email…"). Copy rule: user-facing surfaces say *catching up / checking*, never "drain" (internal name only). Shown while a pull run we kicked is in flight; auto-clears when the gmail-jobs source's `lastRunAt` advances past the kick (polled every 8s) or after a 150s safety cap. Triggered by: the manual **Refresh** button, a fresh **Gmail reconnect** (which also force-runs the scan), and resumed on mount if a recent kick hasn't completed (survives the reconnect OAuth round-trip). The kick timestamp lives in `localStorage['job:lastPullKickAt']`, shared with `pipeline.js` `refreshSources()`.
- `.rail-status` (`<ladder-pipeline-status>`, v2.45) — permanent one-glance email-scan state in the rail, pinned above `.rail-user`. Essentials only: indicator dot + one label (+ count and thin bar while catching up). States: `active` (accent-strong dot, pulsing), `paused` (error dot — reconnect Gmail / add AI credits), `attention` (warning dot, other `lastError`), `idle` (subtle dot, "Up to date"). Color rule: sentiment tokens only for paused/attention; the working state uses the accent family like `.recs-draining`. The element links to the Inbox and never repeats the banner's CTA. Polls `recommendations?health=1` (sourceHealth only, no rec queries) — 60s idle / 12s active.
