...
| Banners & toasts | `.toast-host`, `.recs-health-banner` | Every transient surface needs a defined lifecycle — at least two of: auto-timeout, explicit dismiss, state-driven refresh. Toasts (`job:toast`) render only through the global host in app.js (4.5s timeout, click-dismiss) and support an optional inline action (`{ action, onAction, duration }` — used for Undo confirmations, 8s). Confirmation banners auto-expire (added 12s, liveness 20s). Persistent alerts (source health) get ✕ = 6h signature-keyed snooze + 5-min background revalidation, and clear when the server state clears. Dismissals that must survive reloads persist to localStorage (closed-banner watermark). |
