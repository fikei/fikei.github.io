...
//   role-*           → /ladder/history/roles/{slug}/
//   project-* / *-project → /ladder/history/projects/{slug}/
//   skill-*          → /ladder/history/skills/{slug}/
//   win-*            → /ladder/history/wins/{slug}/
//   otherwise        → /ladder/history/companies/{slug}/  (best-effort)
// A future PR replaces this with a real manifest fetched from kb-read.
export function resolveSlug(slug) {
  const s = slug.toLowerCase().trim();
  if (s.startsWith('role-')) return `/ladder/history/roles/${s}/`;
  if (s.startsWith('project-') || s.endsWith('-project')) return `/ladder/history/projects/${s}/`;
  if (s.startsWith('skill-')) return `/ladder/history/skills/${s}/`;
  if (s.startsWith('win-')) return `/ladder/history/wins/${s}/`;
  return `/ladder/history/companies/${s}/`;
}
