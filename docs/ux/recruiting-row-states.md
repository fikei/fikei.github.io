...
### 2026-08-23 — boot-opt: cached gate verdict (v3.86.0)
Boot telemetry (14-day window, 44 `boot_access`/`boot_access.deep` samples) showed the Discord membership check outpacing loadAll as the slow leg — `boot_access` p75 1068ms vs `boot_data` p75 763ms, ~40% slower — so the access check, not the data load, was the lever. `_checkMembershipAndEnter` now caches a successful verdict in `localStorage` (`agape:gate`, keyed by user id, 6h TTL). On boot, a fresh matching verdict lets the app enter immediately without awaiting the Discord round-trip; the real check still runs in the background and, only on a definite refusal (not linked / not a Recruiting Society member), evicts the cache and sends the user back to the gate. A failed or aborted background re-check never evicts and is never cached — same as the inline path already required.
