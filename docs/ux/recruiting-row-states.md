...
### 2026-08-24 — v3.86.0: boot-opt, cached gate verdict
Daily boot-telemetry review (14-day window, 9 days with data, n=45 on boot_access) found the Discord membership check, not `loadAll()`, as the slower boot leg: `boot_access` p75 1064ms vs `boot_data` p75 790ms — a 35% gap, above the no-clear-winner threshold. `_checkMembershipAndEnter` now caches a successful verdict in `localStorage['agape:gate']` (`{userId, username, isAdmin, at}`) for 6 hours; a matching, unexpired cache lets boot enter immediately while `verifyGateInBackground()` re-checks Discord membership without blocking first paint. An explicit refusal clears the cache and re-shows the gate; a network hiccup or server error is inconclusive and leaves the cached verdict alone — a failed check is never cached.
