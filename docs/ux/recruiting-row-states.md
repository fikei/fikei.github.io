...
### 2026-08-30 — v3.86.0: boot-opt, cached gate verdict
Boot telemetry (7 days, 35 loads) showed `boot_access` p75 1060ms vs `boot_data` p75 766ms — the Discord membership check, not `loadAll()`, was the boot bottleneck. `_checkMembershipAndEnter` now caches a successful access verdict in `localStorage` (`agape:gate`: `{userId, username, isAdmin, at}`). A cached verdict for the current user under 6 hours old lets boot enter immediately, verifying membership against Discord in the background (`verifyGateInBackground`); an explicit refusal there clears the cache and returns the reader to the gate. A refused or failed check is never cached.
