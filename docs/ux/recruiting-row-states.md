...
### 2026-08-25 — v3.86 boot-opt: cached gate verdict
Boot telemetry (`boot_vitals_summary`, 9 days of data, n=36 boots) showed `boot_access` p75 at 1064ms vs `boot_data` p75 at 790ms — the Discord membership gate check, not `loadAll()`, was the slower of the two parallel legs and the clear bottleneck (~35% slower; `.deep` variants had n=9, under the 10-sample bar for using them). `_checkMembershipAndEnter` now caches a successful verdict in `localStorage` (`agape:gate`: `{userId, username, isAdmin, at}`). On boot, a cached verdict for the current user under 6 hours old enters immediately without awaiting the `discord-membership` round-trip; the check still runs in the background, and a refused or failed result clears the cache and drops the user back to the gate. A refused/failed check is never cached.
