...
### v3.75 — 2026-08-19 — cached gate verdict (boot-opt)
Boot vitals (5-day window, n=34) showed `boot_access` p75 at 1166ms against `boot_data` p75 at 868ms — the Discord membership round trip, not `loadAll`, was the slower half of the parallel boot pair. `_checkMembershipAndEnter` now caches a successful verdict in `localStorage` (`agape:gate`, `{userId, username, isAdmin, at}`). A verdict under 6h old for the current user lets boot enter immediately without awaiting the `discord-membership` fetch; the fetch still runs in the background, and a refusal or failure clears the cache and drops the user back to the gate. A refused or failed check is never cached in the first place.
