...
### 2026-08-15 — v3.74 boot-opt: cached gate verdict
Boot telemetry (14-day window, 4 `days_with_data`, n=10 on the `.deep` pair) showed the Discord membership check outweighing `loadAll()`: `boot_access.deep` p75 1647ms vs `boot_data.deep` p75 1298ms — a 27% gap on deep links, the complaint case. `_checkMembershipAndEnter` now caches a successful verdict in `localStorage['agape:gate']` ({userId, username, isAdmin, at}). On boot, a cache entry for the current user under 6 hours old enters immediately while the real `discord-membership` check runs behind it; a real check that disagrees (unlinked, no longer a member, or the call itself fails) clears the cache, hides the app, and bounces back to the gate. A refused or failed check is never cached, and a matching background success rolls the 6-hour window forward.
