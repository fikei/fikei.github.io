...
### 2026-08-21 — cached gate verdict (v3.85.0)
No row-visual change; noting a boot-perf fix here since it touches when rows first appear. Boot-vitals telemetry (8 days, n=24 boots / 10 deep-linked) showed the Discord-membership gate check (`boot_access.deep` p75 1154ms), not `loadAll` (`boot_data.deep` p75 824.5ms), as the slower half of the access/data pair that gates first paint. `_checkMembershipAndEnter` now caches a successful verdict in `localStorage['agape:gate']` (`{userId, username, isAdmin, at}`, 6h TTL); a fresh boot for the same user enters immediately on a cache hit and re-verifies the Discord check in the background — a refusal there clears the cache and drops back to the gate. A refused or failed check is never cached.
