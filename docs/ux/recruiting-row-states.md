...
### v3.86 (2026-08-26) — boot-opt: cached gate verdict
Boot telemetry (9 days of data, n=36 boot_access samples) showed `boot_access` (the Discord membership gate check) as the slower boot leg, not `boot_data` (loadAll): p75 1064ms vs 790ms, a 35% gap — outside the no-clear-winner band. `_checkMembershipAndEnter` now caches a successful access verdict in `localStorage['agape:gate']` (`{userId, username, isAdmin, at}`). On a warm boot — same user, cached under 6h ago — the app enters immediately instead of awaiting the `discord-membership` fetch, while the real check still runs and reconciles in the background: a refusal or failure there clears the cache and tears the app back down to the gate. A refused or failed check is never cached.
