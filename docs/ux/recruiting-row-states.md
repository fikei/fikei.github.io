...
### v3.86.0 (2026-08-22) — cached gate verdict speeds up boot
Boot vitals (14-day window, n=35) showed `boot_access` — the Discord membership check — running p75 1068ms vs. `boot_data` (loadAll) at p75 763ms, ~40% slower and the clear bottleneck since access and data already load in parallel. A successful verdict is now cached in `localStorage` (`agape:gate`, keyed to the signed-in user id) for 6 hours; on a repeat boot within that window the app enters immediately once `loadAll` finishes, while the real Discord check confirms silently in the background. A background refusal (unlinked, no longer a Recruiting Society member, or the check itself failing) clears the cache and kicks back out to the gate — a refused or failed check is never cached. `_checkMembershipAndEnter` in `applications/js/app.js` now shares its post-verdict entry sequence (`enter()`) between the cached fast path and the normal network path so they can't drift apart.
