...
### v3.77 — cached gate verdict (boot-opt, 2026-08-20)
Boot telemetry (`boot_vitals_summary`, 14-day window) showed the Discord access check — not the data load — is the slow leg of boot: **boot_access.deep p75 1260ms vs boot_data.deep p75 915ms** (plain variants agreed: 1076ms vs 833ms, 37.7% / 29% apart). `loadAll` already runs parallel to the access check, so the access check was the only lever left.
- On a successful access verdict, `_checkMembershipAndEnter` now caches `{userId, username, isAdmin, at}` in `localStorage['agape:gate']`. On the next boot, if the cached verdict matches the current user id and is under 6h old, the app **enters immediately on the cached verdict** — first paint is now bounded by `loadAll`, not the Discord round-trip — and re-verifies against Discord in the background (`_verifyAccessInBackground`).
- If the background check comes back **refused**, the cache is cleared, the app hidden, and the gate shown. A **failed/transient** check (network error, non-200, abort) keeps the cached verdict and is never cached. A refused or failed check is never cached.
- The "we're in" body was extracted to `_enterApp(status, user, …)`, shared by the verified and cached paths; the cached path reports a ~0 `boot_access` phase since it never waited for the fetch.
