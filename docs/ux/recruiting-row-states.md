...
### 2026-08-31 — v3.86.0: cached gate verdict skips the discord-membership round trip on warm boot
Boot telemetry (`boot_vitals_summary`, 14-day window, n=29 boot cycles) showed `boot_access` (the Discord Recruiting Society membership check) at p75 1060ms, running ~38% slower than `boot_data` (`loadAll`) at p75 766ms — the access check, not data loading, was the boot-time lever. `_checkMembershipAndEnter` now caches a successful verdict in `localStorage['agape:gate']` as `{userId, username, isAdmin, at}`. On boot, a verdict for the current user under 6h old lets the app enter immediately without awaiting the `discord-membership` fetch — `loadAll()` was already kicked off in parallel and is unaffected — while that fetch still runs in the background (`verifyGateInBackground`) and boots the user back to the gate if it comes back refused. Only a linked + member verdict is ever cached; a refused or failed check is never written to the cache.
