...
### 2026-08-18 — v3.75.0: cached gate verdict cuts boot latency
Boot-vitals telemetry (14-day window, 6 days with data, n=27 boot_access / n=10 deep-link) showed the Discord membership check outweighing the data load on deep links — p75 `boot_access.deep` 1647ms vs `boot_data.deep` 1256ms, a 31% gap. A returning housemate now enters on a cached access verdict (`localStorage['agape:gate']`, 6-hour TTL, keyed to the signed-in user id) without awaiting that round-trip; the network check still runs in the background and evicts the cache + re-gates on an explicit refusal (never on a network failure or timeout). A fresh sign-in still waits for the live check, same as before.
