...
| **Accept applicant-provided schedulers (cal.com / Calendly)** — When a reply contains a scheduling link instead of prose availability, detect it (the intent extractor already sees the body), surface it on the row/profile as "they sent their scheduler", and let a housemate book directly through it — for tours, offer the link to the housemates who reacted on the poll. Counterpart to v3.64's removal of *our* outbound scheduling link: we no longer send one, but we should gracefully consume theirs. | Pending |
