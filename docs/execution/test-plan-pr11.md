...
|---|-----------|-------|-----------------|-------|
| 1.1 | Add Spotify link | Paste a `open.spotify.com/track/...` URL into Boards | Auto-categorizes to "listen" category | |
| 1.2 | Add SoundCloud link | Paste a `soundcloud.com/artist/track` URL | Auto-categorizes to "listen" | |
| 1.3 | Add Bandcamp link | Paste a `artist.bandcamp.com/album/...` URL | Auto-categorizes to "listen" | |
| 1.4 | Add music.youtube.com link | Paste a `music.youtube.com/watch?v=...` URL | Auto-categorizes to "listen" (not "watch") | |
| 1.5 | Add regular youtube.com link | Paste a `youtube.com/watch?v=...` URL | Categorizes to "watch" (NOT "listen") | |
| 1.6 | Filter by "listen" | Click "listen" in category nav | Only music links shown; sub-tags appear (genres, moods, formats) | |
...
| 1.10 | Listen widgets render | View "listen" category with 3+ music links | Eligible widgets appear: `profiler`, `discover-more`, `upcoming-releases`, `sound-shelf`, `listen-next`, `board-overview` | |
...
| # | Test Case | Steps | Expected Result | Notes |
|---|-----------|-------|-----------------|-------|
| 2.1 | Category switch clears widgets | View "wear" → wait for widget → switch to "home" | Widget regenerates with ONLY home items; no wear-category content visible | |
| 2.2 | New link isolated to category | In "wear", add a clothing link → switch to "eat" | Eat widgets do NOT reference the new wear link | |
...
| # | Test Case | Steps | Expected Result | Notes |
|---|-----------|-------|-----------------|-------|
| 3.1 | Profiler → wear | View "wear" category with 3+ items | `style-summary` server widget renders (fashion analysis) | |
| 3.2 | Profiler → home | View "home" category with 3+ items | `design-dna` server widget renders (interior design) | |
| 3.3 | Profiler → watch | View "watch" category with 3+ items | `viewer-profile` server widget renders | |
| 3.4 | Profiler → eat | View "eat" category with 3+ items | `flavor-profile` server widget renders | |
...
| 3.7 | Profiler → read | View "read" category with 3+ items | `reader-identity` server widget renders | |
| 3.8 | Profiler → use | View "use" category with 3+ items | `setup-profile` server widget renders | |
| 3.9 | min_items eligibility | View category with only 2 items (widget requires 3) | Widget does NOT appear | |
...
| # | Test Case | Steps | Expected Result | Notes |
|---|-----------|-------|-----------------|-------|
| 4.1 | Engine loads on page load | Open Boards, open DevTools console | Console: `[design-constraints] Ready — X components, Y widget atoms, Z templates` | |
| 4.2 | Ready state | In console, run `window.DS_CONSTRAINTS.ready` | Returns `true` | |
...
| 4.4 | Audit — no violations | In console, run `auditDesignConstraints()` | Returns empty array; console: `[design-constraints] ✓ No violations found` (green) | |
...
| # | Test Case | Steps | Expected Result | Notes |
|---|-----------|-------|-----------------|-------|
| 8.1 | Default view | Open Events page | Table view active; "Table" button highlighted | |
| 8.2 | Switch to Day view | Click "Day" button | Day view renders; title shows day name + date (e.g., "Monday, January 15, 2026"); navigation: ← Today → | |
| 8.3 | Switch to Week view | Click "Week" button | Week view renders; title shows date range (e.g., "Jan 12 – Jan 18, 2026"); 7-column grid (Sun–Sat) | |
| 8.4 | Switch to Month view | Click "Month" button | Month calendar renders; 7×6 grid; today cell highlighted | |
...
| # | Test Case | Steps | Expected Result | Notes |
|---|-----------|-------|-----------------|-------|
| 12.1 | Default state | View variant with no audit history | Green stoplight; "Comment" button visible | |
...
| # | Test Case | Steps | Expected Result | Notes |
|---|-----------|-------|-----------------|-------|
| 15.1 | Migration applies cleanly | Run `supabase db push` or apply manually | No errors; tables created | |
