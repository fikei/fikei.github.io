...
console.log(`[applications] v${VERSION} - cached gate verdict skips discord-membership round trip on warm boot`);
...
// boot-opt: cached gate verdict. 14-day telemetry (n=29 boot cycles) showed
// boot_access p75 1060ms running ~38% slower than boot_data p75 766ms — the
// discord-membership round trip was the lever, not loadAll. A verdict under
// 6h old lets boot skip awaiting that fetch entirely; the fetch still runs
// in the background and a live refusal still boots the user out. A refused
// or failed check is never written to this cache.
...
function readGateCache(userId) {
  try {
    ...
    if (parsed && parsed.userId === userId && typeof parsed.at === 'number' &&
        (Date.now() - parsed.at) < GATE_CACHE_MAX_AGE_MS) {
      return parsed;
    }
  } catch { /* corrupt cache — treat as a miss */ }
  return null;
}

function writeGateCache(userId, username, isAdminFlag) {
  try {
    localStorage.setItem(GATE_CACHE_KEY, JSON.stringify({ userId, username, isAdmin: isAdminFlag, at: Date.now() }));
  } catch { /* storage unavailable/full — cache is a speed-up, not a requirement */ }
}

function clearGateCache() {
  try { localStorage.removeItem(GATE_CACHE_KEY); } catch {}
}

// Re-checks the real verdict behind a cached entry. A live refusal must
// always win over a stale cache, so this can still boot the user back to
// the gate after enterApp() already let them in.
function verifyGateInBackground(token, user) {
  fetch(`${SUPABASE_URL}/functions/v1/discord-membership`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}` },
    body: JSON.stringify({ action: 'status' }),
  }).then(async (resp) => {
    if (!resp.ok) return; // transient failure — don't punish the cached session
    ...
    // Still good — refresh the cache so the 6h window resets from a live check.
    writeGateCache(user.id, status.discordUsername, status.isRecruitingAdmin === true);
  }).catch(() => { /* network hiccup — leave the cached session alone */ });
}

// Shared "we have a verdict, now load data and render" tail for both the
// fresh-fetch path and the cached-verdict fast path.
async function enterApp({ tEnter, tAccess0, dataP, getTData, username, isAdminFlag }) {
  // in — identify self, load data, render
  const user = window.CtrlAuth.getUser();
  me = { id: user.id, name: username || user.email || 'Housemate', groupEmail: user.email || null };
  ...
  /* Boot report — one console line, and each phase into the vitals
     ...
     user's real wait from tapping the link. A cached verdict skips the
     discord round trip, so boot_access collapses toward ~0 here. */
  {
...
async function _checkMembershipAndEnter() {
  ...
  const cached = readGateCache(authedUser?.id);
  if (cached) {
    verifyGateInBackground(token, authedUser);
    await enterApp({
      tEnter, tAccess0, dataP, getTData: () => tData,
      username: cached.username, isAdminFlag: cached.isAdmin,
    });
    return;
  }
  ...
  try {
    ...
    // in — cache the verdict so the next boot can skip this round trip.
    // Only a linked + member verdict ever reaches here, so a refused or
    // failed check is never cached.
    writeGateCache(authedUser.id, status.discordUsername, status.isRecruitingAdmin === true);
    await enterApp({
      tEnter, tAccess0, dataP, getTData: () => tData,
      username: status.discordUsername, isAdminFlag: status.isRecruitingAdmin === true,
    });
  } catch (e) {
